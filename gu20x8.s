...
GPLOT:	LDA #$01		; Turn the Y coord into a bit mask
GPLOT1:	DEY
...
GPLOT3:	LDA REDBUFF,X		; Now we get to the red bits
...
GPLOT4:	RTS			; We're done plotting this pixel
	
	;; Get the color of a pixel at location defined by registers X Y
	;; Returns the color in A
GCOLOR:	LDA #$01		; Get a bit mask from the Y coord
GCOLOR1:DEY
...
GCOLOR3:LDA REDBUFF,X		; Now get the red column
	...
	RTS
GCOLOR5:TYA			; Move the blue (or black) pixel to A
	RTS			; Return with color in A

	;; Fill the display with the current plot color
GFILL:	LDA #$00		; First clear the blue and red buffers to black
	LDX #$13
GFILL1:	STA BLUEBUFF,X
	STA REDBUFF,X
	DEX
	BPL GFILL1
...
GFILL2:	STA BLUEBUFF,X		; Fill the buffer
	DEX
	BPL GFILL2		; Loop until done
GFILL3:	LDA COLOR		; Test for red or white
	AND #$02
	BEQ GFILL5		; Not red or white so return
	LDA #$FF		; The red fill byte
	LDX #$13		; Buffer length
GFILL4:	STA REDBUFF,X		; Fill the buffer
	DEX
...
GVLINE:	PHY			; Save the Y coord for later
...
GVLINE4:JMP GPLOT2		; Jump into the mask saving step of GPLOT to draw a full line
...
GBRITE:	LDA #$0E		; Set command mode bit
	STA ORB
